...
const AssetTab = ({ strategyId, mode, results, isRunning, onRun, onStop, config, onSaveConfig }) => {
    ...
    useEffect(() => {
        let interval;
        if (isRunning && selectedLogDate) {
            interval = setInterval(() => {
                fetchLogs(selectedLogDate);
            }, 2000);
        }
        return () => clearInterval(interval);
    }, [isRunning, selectedLogDate]);
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* Status Banner */}
            {isRunning && (
                <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-xl flex items-center shadow-sm animate-pulse">
                    <i data-lucide="loader-2" className="w-5 h-5 mr-3 animate-spin"></i>
                    <span className="font-medium">Backtesting in progress... The agent is analyzing market data and executing trades.</span>
                </div>
            )}
            ...
            {/* Split View: Chart & Logs */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[600px]">
                {/* Chart */}
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col h-full">
                    <h3 className="text-lg font-semibold text-gray-800 mb-4 border-b pb-2 shrink-0">📈 Asset Evolution</h3>
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col h-full">
                    ...
                    <div className="logs-container flex-1 overflow-y-auto bg-gray-50 rounded-lg p-4 border border-gray-200 font-mono text-xs leading-relaxed">
                        {logs.length > 0 ? (
                            logs.map((log, idx) => (
